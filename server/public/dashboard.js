...
const messages={chrome_unavailable:'Не удалось открыть страницу входа. Скопируйте ссылку и откройте её в браузере.',chrome_open_failed:'Не удалось открыть страницу входа. Скопируйте ссылку и откройте её в браузере.',duplicate_email:'Сотрудник с такой почтой уже существует.',invalid_employee:'Проверьте имя и рабочую почту.',active_key_exists:'У сотрудника уже есть действующий ключ.',wrong_account:'Выбран другой аккаунт. Повторите вход в прежний аккаунт подписки.',duplicate_account:'Этот аккаунт уже подключён к окружению.',login_port_in_use:'Уже выполняется другой вход Codex. Завершите его и повторите.',login_in_progress:'Вход уже открыт. Завершите его или отмените.',login_expired:'Время ожидания закончилось. Начните вход заново.',login_start_failed:'Не удалось начать вход OpenAI. Попробуйте снова.',device_login_disabled:'Для входа по коду включите Device code authorization в настройках безопасности ChatGPT, затем повторите попытку.',login_failed:'OpenAI не подтвердил вход. Попробуйте снова.',account_verification_failed:'Не удалось подтвердить аккаунт.',subscription_requires_login:'Подписка требует повторного входа.',internal_error:'Не удалось выполнить действие. Повторите попытку.'};
...
function showKey(employeeId,raw){const e=employeeById(employeeId);openModal(modalHead('Ключ готов',e?e.name+' · '+e.email:'')+'<div class="modal-body"><div class="modal-success">'+icon('check')+'</div><div class="key-display" id="new-key">'+esc(raw)+'</div><button class="btn btn-primary key-copy" data-action="copy-key">'+icon('copy')+'Скопировать ключ</button><p class="one-time">'+icon('lock')+'<span>Скопируйте сейчас. После закрытия полный ключ будет недоступен.</span></p><div class="new-key-note">Ключ бессрочный: действует до ручного отзыва и открывает доступ ко всем подпискам окружения.</div></div><div class="modal-footer"><button class="btn btn-secondary" data-action="close">Готово</button></div>');revealedKey=raw;}
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    el.disabled=true;
    if(action==='copy-setup'){await navigator.clipboard.writeText($(el.dataset.target).textContent);toast('Скопировано');}
...
document.addEventListener('submit',async event=>{
  if(event.target.id!=='add-form')return;event.preventDefault();const submit=event.target.querySelector('[type="submit"]');submit.disabled=true;
  try{const result=await api('/api/employees',{method:'POST',body:{name:$('employee-name').value,email:$('employee-email').value,issueNow:$('issue-now').checked}});await closeModal();await load();if(result.key)showKey(result.employee.id,result.key);else toast('Сотрудник добавлен');}
  catch(error){if($('form-error')){$('form-error').textContent=error.message;$('form-error').hidden=false;}else showError(error);}finally{submit.disabled=false;}
});
...
async function boot(){hydrate();const session=await api('/api/session');csrf=session.csrf;gatewayUrl=session.gatewayUrl;$('provider-config').textContent="[model_providers.mox]\nname = \"MOX\"\nbase_url = \"__GATEWAY__\"\nwire_api = \"responses\"\nsupports_websockets = false\n\n[model_providers.mox.auth]\ncommand = \"/bin/sh\"\nargs = ['-c', '/bin/cat \"$HOME/.codex/mox-access-key\"']\ntimeout_ms = 5000\nrefresh_interval_ms = 0".replace('__GATEWAY__',gatewayUrl);$('save-key-command').textContent="zsh -c 'set -e\numask 077\nmkdir -p \"$HOME/.codex\"\nread -rs \"mox_key?Вставьте ключ MOX и нажмите Enter: \" || exit 1\nprint\nif [[ ! \"$mox_key\" =~ \"^mox_[0-9a-f]{64}$\" ]]; then\n  print \"Нужен полный ключ MOX: mox_ и 64 символа после него.\"\n  exit 1\nfi\nprintf \"%s\" \"$mox_key\" > \"$HOME/.codex/mox-access-key\"\nchmod 600 \"$HOME/.codex/mox-access-key\"\nunset mox_key\nprint \"Ключ сохранён.\"'";await load();setView(state.connections.length?'employees':'subscriptions');const pending=await api('/api/auth/active');if(pending){auth=pending;authProgress();void pollAuth(++authGeneration).catch(showError);}}
