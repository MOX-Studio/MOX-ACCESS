...
export function appendUsage(records, record) {
  ...
  if (existing) {
    for (const field of ['employeeId', 'connectionId', 'completedAt', 'inputTokens', 'outputTokens']) {
      if (existing[field] !== record[field]) throw new Error('Conflicting usage event');
    }
    return false;
  }
...
export function aggregateUsage(employees, records, period = 'day', now = Date.now()) {
  const range = periodRange(period, now), totals = emptyTotals(), seen = new Set();
  const rows = new Map(employees.map(employee => [employee.id, { employee, ...emptyTotals() }]));
  for (const event of records) {
    if (seen.has(event.id) || event.completedAt < range.start || event.completedAt >= range.end) continue;
    seen.add(event.id);
    if (!rows.has(event.employeeId)) {
      rows.set(event.employeeId, { employee: { id: event.employeeId, name: 'Архивный сотрудник', email: '', color: 0 }, ...emptyTotals() });
    }
    countInto(totals, event);
    countInto(rows.get(event.employeeId), event);
  }
...
export function demoUsage(employees, connections, now = Date.now()) {
  ...
  for (let daysAgo = 0; daysAgo < 35; daysAgo++) {
    employees.forEach((employee, index) => {
      ...
      appendUsage(records, {
        id: 'demo-seed-' + daysAgo + '-' + employee.id, employeeId: employee.id,
        connectionId: connections[(daysAgo + index) % connections.length].id,
        completedAt, receivedAt: completedAt, source: 'demo',
        inputTokens: 12_000 + ((index * 1379 + daysAgo * 859) % 38_000),
        outputTokens: 1800 + ((index * 347 + daysAgo * 193) % 6800)
      });
    });
  }
  appendUsage(records, {
    ...
    inputTokens: 1024, outputTokens: null
  });
  return records;
}
